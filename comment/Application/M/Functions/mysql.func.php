...
function connect1($host,$user,$password,$charset,$database) {
    ...
    mysqli_select_db ( $link, $database ) or die ( 'ָ指定数据库打开失败<br/>ERROR ' . mysqli_errno ( $link ) . ':' . mysqli_error ( $link ) );
    return $link;
}
/**
 * 连接 需要传递数组
...
function connect2($config) {
    ...
    mysqli_select_db ( $link, $config ['dbName'] ) or die ( 'ָ指定数据库打开失败<br/>ERROR ' . mysqli_errno ( $link ) . ':' . mysqli_error ( $link ) );
    return $link;
}
/**
 * 用常量的形式建立连接
...
function connect3(){
    ...
    mysqli_select_db ( $link, DB_DBNAME ) or die ( 'ָ指定数据库打开失败<br/>ERROR ' . mysqli_errno ( $link ) . ':' . mysqli_error ( $link ) );
    return $link;
}
/**
 * 插入操作
...
function fetchAll($link, $query, $result_type = MYSQLI_ASSOC) {
    $result = mysqli_query ( $link, $query );
    if ($result && mysqli_num_rows ( $result ) > 0) {
        while (($row = mysqli_fetch_array( $result, $result_type ))>0) {
            $rows [] = $row;
        }
        return $rows;
    } else {
